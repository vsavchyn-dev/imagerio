...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) {
        std::cout << "No input file provided to program...\n"
            ...
            << "\t-f=<valid_filter> or -filter=<valid_filter>     name of the filter that will be applied to the image  \n"
            << "\tsupported modes:\n"
    ...
    for (int x = 1; x < argc || earlyexit;) {
        int flag = starts_with("-o", argv[x]) * o +
            (starts_with(argv[x], "-f") || starts_with(argv[x], "-filter")) * f +
            (starts_with("-h", argv[x]) || starts_with("-help", argv[x])) * h +
            (starts_with("-i", argv[x]) || starts_with("-image", argv[x])) * i + 
            (starts_with("-p", argv[x]) || starts_with("-parallel", argv[x])) * p;
        ...
        case o:
            if (is_valid_path(argv[x + 1]) && is_valid_extension(argv[x + 1], valid_output_ext)) {
                ...
                std::cout << "Output path is invalid or extension is not one of possible ones!\n";
                earlyexit = true;
            }
            break;
        case f: 
            if (is_valid_extension(argv[x], valid_filters)) {
                ...
                const std::string filter_name = filter_name_unparsed.substr(eq_sign_pos + 1, filter_name_unparsed.size() - eq_sign_pos);

                auto iter = std::find(valid_filters.begin(), valid_filters.end(), filter_name);
                const size_t idx = std::distance(valid_filters.begin(), iter);
                filter = filters_enum(idx);

                x += 1;

                // std::cout << "Unparsed name: " << filter_name_unparsed << "\n"
                //         << "Parsed name: " << filter_name << "\n"
                //         << "Position of eq sign: " << eq_sign_pos << "\n"
                //         << "Calculated index: " << idx << "\n"
                //         << "Final filter decision:" << filter << "\n";
            }
            else {
                std::cout << "Invalid filter name!\n";
                earlyexit = true;
            }
    ...
    switch (filter) {
        case filters_enum::gaussian_blur:
            parallel_impl ? imgr::GaussianBlur::apply_gaussian_blur_parallel(og_img) : imgr::GaussianBlur::apply_gaussian_blur(og_img);
            break;
